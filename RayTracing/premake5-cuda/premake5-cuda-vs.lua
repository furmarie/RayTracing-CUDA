...
local function writeTableAsOneString(property, values, sep)
    if values ~= nil then
        writeString(property, table.concat(values, sep))
    end
end
...
local function addCompilerProps(cfg)
    ...
    writeTableAsOneString('CodeGeneration', cfg.cudaCodeGeneration, ';')
